...
sudo sed -i -e "s/SID=\".*\"/SID=\"$LocalSID\"/g" \
	-e "s/sambaDomain=\".*\"/sambaDomain=\"jaime\"/g" \
	-e "s/slaveLDAP=/#slaveLDAP=/g" \
	-e "s/masterLDAP=\".*\"/masterLDAP=\"ldap:\/\/ldap-script.jaime.local\/\"/g" \
	-e "s/ldapTLS=\"1\"/ldapTLS=\"0\"/g" \
	-e "s/verify=\"require\"/verify=\"none\"/g" \
	-e "s/clientcert=\".*\"/clientcert=\"\/etc\/smbldap-tools\/smbldap-tools.jaime.local.pem\"/g" \
	-e "s/clientkey=\".*\"/clientkey=\"\/etc\/smbldap-tools\/smbldap-tools.jaime.local.key\"/g" \
	-e "s/suffix=\".*\"/suffix=\"dc=jaime,dc=local\"/g" \
	-e "s/userSmbHome=\".*\"/userSmbHome=\""'\\\\'"jaime"'\\%U'"\"/g" \
	-e "s/userProfile=\".*\"/userProfile=\""'\\\\'"jaime"'\\profiles\\%U'"\"/g" \
	-e "s/userHomeDrive=\".*\"/userHomeDrive=\"H:\"/g" \
	-e "s/mailDomain=\".*\"/mailDomain=\"jaime.local\"/g" \
...
sudo sed -i -e "s/slaveDN=\"/#slaveSID=\"/g" \
	...
	-e "s/masterDN=\".*\"/masterDN=\"cn=admin,dc=jaime,dc=local\"/g" \
...
read -p "Se va a instalar libnss-ldap, lo siguiente que debes escribir en orden es:
    ldap://ej..'192.168.100.0'
    dc=jaime,dc=local
    3
    Sí
    No
    cn=admin,dc=jaime,dc=local
    ******"
...
@echo off
net time \\\\ej..'192.168.100.0' /set /yes
net use z: \\\\ej..'192.168.100.0'\datosEnServidor
EOF"
...
portmap: ej. '192.168.100.0'/24
nfs: ej. '192.168.100.0'/24
EOF"
sudo bash -c "echo \"/datosDeUsuariosLDAP ej. '192.168.100.0'/24(rw,sync,no_root_squash,no_subtree_check)\" >> /etc/exports"
