...
function ipestatica () {
	...
	function grabar_configuracion(){
		...
		read -p "Ahora se va a reiniciar el ordenador, a continuacion vuelva a ejecutar el script y siga con el paso 2"
		#sudo reboot
	menu
...
function addhosts () {
    ...
    if [ -n "$(grep $HOSTNAME /etc/hosts)" ] && [ -n "$(grep $IP /etc/hosts)" ] ### Comprueba si existe alguno de los dos parametros
        then ### Si existe alguno de los dos informa
            echo "$HOSTNAME y $IP already exists : $(grep $HOSTNAME $ETC_HOSTS)"
        else ### Sino existe
	...
	read -p "Ahora se va a reiniciar el ordenador, a continuacion vuelva a ejecutar el script y siga con el paso 3"
		#sudo reboot
	menu
}
